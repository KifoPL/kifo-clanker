...
module.exports = {
	name: "todo",
	description: "This command allows you to create simple to-do notes.",
	usage: [
	...
	async execute(message, args) {
		...
		if (args[0].match(kifo.urlRegex())) {
			...
			msg = await message.client.guilds
				.resolve(guildId)
		...
		if (isLink)
			Embed.addField(
				"Link:",
				`[Click for original message!](${msg.url})`
			);
		Embed.addField(
			"Finished?",
			...
			.then((message1) => {
				...
				message
					.reply({
						embeds: [
							kifo.embed(
								"Knock knock, I just left you a note in DMs!"
							),
						],
					})
					.catch(() => {});
