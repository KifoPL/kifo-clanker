			...
			if (!args[1]) {
				...
				if (entity != null || mention != null) {
					...
					if (entity != null) {
						...
						entity = message.guild.members.resolve(
							kifo.mentionTrim(args[0])
						);
						if (entity != null) {
							whatami = "member";
				...
				if (whatami == "channel") {
					...
					if (
						!message.channel.permissionOverwrites.resolve(entity.id)
					) {
					...
					if (ms(time) < 1000 * 60)
						return message
							.reply({
								embeds: [
									kifo.embed(
										"Set the time to at least a minute!"
									),
								],
							})
							.catch(() => {});
				...
				if (time !== undefined) {
					await message.reply({
						content: `I'll revert the perms at <t:${Math.floor(
							end.getTime() / 1000
