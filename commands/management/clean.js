...
async function cleanChannel(message, channel, callback) {
	//PERM CHECK
	if (!message.member.permissionsIn(message.channel).has(Discord.Permissions.FLAGS.MANAGE_CHANNELS)) {
		callback(new Error("You don't have `MANAGE_CHANNELS` permission!"), {
			isSuccess: false,
		});
		return;
	}
	if (!message.member.permissionsIn(message.channel).has(Discord.Permissions.FLAGS.MANAGE_ROLES)) {
		callback(new Error("You don't have `MANAGE_ROLES` permission!"), {
			isSuccess: false,
		});
		return;
	}
	if (!message.guild.me?.permissionsIn(message.channel).has(Discord.Permissions.FLAGS.MANAGE_ROLES)) {
		callback(new Error("I don't have `MANAGE_ROLES` permission!"), {
			isSuccess: false,
		});
		return;
	}
	...
	try {
		await channel.permissionOverwrites.cache.each(async (permOver) => {
...
async function cleanPerms(message, permOver, callback) {
	if (
		permOver.allow.toArray().length === 0 &&
		permOver.deny.toArray().length === 0
	) {
		await permOver
			.delete() //${message.author.tag}`)
			.then(() => {
				callback(undefined, { isSuccess: true });
			})
			.catch((err) => {
				message.reply({
					embeds: [
						kifo.embed(
							`Could not delete permission overwrite!\n${err}`,
							"Error!"
						),
					],
				});
	...
	//!kifo clean args[0]
		...
		if (args[0].toLowerCase() == "all") {
			...
			await message.guild
				.fetch()
				.then((guild) =>
					guild.channels.cache.each(async (ch) => {
						channelsToClean++;
						await cleanChannel(message, ch, function (err, result) {
							result.isSuccess
								? channelsCleaned++
								: channelsErrors++;
							outputMsg += `${ch.id}\t${ch.name}\t${result.isSuccess
								? "Success"
								: `Error\t${err.message}`
								}\n`;
						});
